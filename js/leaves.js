...
var Leaves = (function () {
  ...
  function renderList(containerId) {
    ...
    // استخدام الـ promise المشترك مع الداشبورد إن كان متاحاً (يمنع طلبين متزامنين)
    var _lvReq = (typeof Dashboard !== 'undefined' && Dashboard.getLeaveReqsShared)
      ? Dashboard.getLeaveReqsShared()
      : API.getLeaveReqs();

    _lvReq.then(function(res) {
      if (!res.ok) {
        var errMsg = res.error === 'timeout'
          ? 'انتهت مهلة الاتصال — يرجى المحاولة مجدداً'
          : 'تعذّر التحميل';
        el.innerHTML = '<div class="empty-state" style="padding:32px">' + errMsg +
          '<br><br><button class="btn-outline" onclick="App.navigate(\'leaves\')">إعادة المحاولة</button></div>';
        return;
      }
  ...
  function renderForm(containerId) {
    ...
    html += '<div id="lf-capacity-warn" class="form-warning" style="display:none"></div>';
  ...
  function _checkCapacity() {
    ...
    _capCheckTimer = setTimeout(function() {
      ...
      API.getRegionInfo(startEl.value, endEl.value, empId).then(function(res) {
        if (!warnEl) return;
        if (!res.ok || (!res.pendingCount && !res.approvedCount)) {
          warnEl.style.display = 'none'; return;
        }
        var parts = [];
        if (res.approvedCount) {
          parts.push('✅ ' + res.approvedCount + ' موظف معتمدة إجازتهم في هذه الفترة' +
            (res.approvedNames && res.approvedNames.length ? ' (' + res.approvedNames.join('، ') + ')' : ''));
        }
        if (res.pendingCount) {
          parts.push('⏳ ' + res.pendingCount + ' طلب قيد المراجعة عند المشرف' +
            (res.pendingNames && res.pendingNames.length ? ' (' + res.pendingNames.join('، ') + ')' : ''));
        }
        warnEl.innerHTML = '<b>⚠️ منطقة ' + (res.region || '') + ':</b><br>' + parts.join('<br>') +
          '<br><small style="opacity:.8">يمكنك الإرسال — المشرف سيقرر عند المراجعة</small>';
        warnEl.style.display = 'block';
      }).catch(function() { if (warnEl) warnEl.style.display = 'none'; });
    }, 700);
  }
  ...
  return {
    ...
    _typeChange, _calcDays, _checkCapacity, _loadShiftEmployees, _showShiftWa, _leaveApproveAndNotify,
