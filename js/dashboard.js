...
var Dashboard = (function () {
  ...
  // Promise مشترك لـ getLeaveReqs — يُشارَك فقط أثناء كونه pending
  // إذا اكتمل، يُطلب طلب جديد (بيانات حديثة)
  var _lvPromise  = null;
  var _lvSettled  = false;

  function _getLeaveReqsShared() {
    if (_lvPromise && !_lvSettled) return _lvPromise;
    _lvSettled = false;
    _lvPromise = API.getLeaveReqs().then(function(r) { _lvSettled = true; return r; });
    return _lvPromise;
  }

  function _invalidateLvCache() { _lvPromise = null; _lvSettled = false; }
  ...
  function _renderManager(el, role, user) {
    ...
    Promise.all([
      ...
      // ملاحظة: _loadPresenceDefaults تُستدعى بعد اكتمال Phase 2 لتجنب طلبين متزامنين

      // ----
      // المرحلة الثانية: طلب واحد مشترك — يُعاد استخدامه في صفحة الإجازات مباشرةً
      // بدلاً من طلب ثانٍ مستقل حين ينتقل المشرف للإجازات
      // ----
      var _el = el;
      var _role = role, _user2 = user, _rgList = rgList, _today = today;
      var _isSup = isSup;
      _getLeaveReqsShared().then(function(r2) {
        ...
        if (_isSup) {
          ...
          // تحميل إعدادات الحضور بعد اكتمال Phase 2 — لا طلبات متزامنة
