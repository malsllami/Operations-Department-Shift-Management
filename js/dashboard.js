...
var Dashboard = (function () {
  ...
  // كاش لـ leaveReqs — يُملأ من getDashboard مباشرةً بدون طلب منفصل
  var _lvPromise = null;
  var _lvSettled = false;
  var _lvCacheTs = 0;
  var _LV_TTL    = 120000; // دقيقتان

  function _getLeaveReqsShared() {
    if (_lvPromise && !_lvSettled) return _lvPromise;               // جارٍ: شارك نفس الـ promise
    if (_lvPromise && _lvSettled && (Date.now() - _lvCacheTs) < _LV_TTL) return _lvPromise; // كاش حديث
    _lvSettled = false;
    _lvCacheTs = Date.now();
    _lvPromise = API.getLeaveReqs().then(function(r) { _lvSettled = true; return r; });
    return _lvPromise;
  }

  function _preloadLeaveReqs(data) {
    _lvPromise = Promise.resolve({ ok: true, data: data });
    _lvSettled = true;
    _lvCacheTs = Date.now();
  }

  function _invalidateLvCache() { _lvPromise = null; _lvSettled = false; _lvCacheTs = 0; }
  ...
  function _renderManager(el, role, user) {
    ...
    Promise.all([
      ...
      var lvReqs  = dash.leaveReqs || [];   // مضمّنة في getDashboard — لا طلب منفصل
      _preloadLeaveReqs(lvReqs);
      var colors  = dash.colors || {};
      var todayShifts = dash.todayShifts || {};
      var shiftStats  = dash.shiftStats  || {};
      ...
      // البيانات مضمّنة في getDashboard — لا Phase 2 ولا طلب إضافي لـ getLeaveReqs
  ...
  return { render: render, quickComprehensive: quickComprehensive,
           ...
           getLeaveReqsShared: _getLeaveReqsShared,
           preloadLeaveReqs: _preloadLeaveReqs,
           invalidateLvCache: _invalidateLvCache };
})();
